...
# ISOM symboly + popis (v pořadí kategorií: terén → vegetace → skály → voda → komunikace
# → stavby → mosty). Sloupec „Symbol" má kód s tečkou tam, kde ISOM dělí sub-symboly.
SYMBOLS = [
    # Terén (§4.5, §4.10)
    ("101",   "Contour"),
    ("102",   "Index contour"),
...
def _normalize_code(sym) -> str:
    ...
    if sym is None:
        return ""
    if sym == 5122:               # ISOM_FOOTBRIDGE alias
        return "512.2"
