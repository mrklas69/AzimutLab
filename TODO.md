...
- [!] **Doplnit CHYBĚJÍCÍ relevantní vrstvy ZABAGED** (Sez. 23, uživatel „stojíme o všechna data z geoportálu, ne jen vybraná"). **Katalog VŠECH 149 vrstev: `docs/kb/zabaged-isom-catalog.md`** — akční seznam kandidátů (◐). **HOTOVO Sez. 24: el. vedení (`Elektrické_vedení` → ISOM 510, oprava z mylného 516=Fence) + sloupy (`Stožár_elektrického_vedení` → příčky).** Zbývá (pořadí dle vizuální návratnosti): `Most` (512, linie ne bod) + `Lávka` (512.2), `Osamělý_balvan`/`Skalní_útvary` (204/201), `Lesní_průsek` (508), bodové orient. prvky (kříž 530/mohyla 526/bunkr/věž 524), `Zeď` (513). Každý = vrstva + mapování + render + verify. Selektivní fetch = chybějící objekty = větší domain gap feederu.
...
- [!] **Pravoúhlost budov — generalizace TVARU (L1 rozšíření, fokus Sez. 23 „podle not uživatele").** Lidská obydlí ≈ 99 % obdélníky; reálné ZABAGED footprinty nesou šikmé/zaoblené hrany a Douglas-Peucker (L1, Sez. 18) je nenarovná na pravé úhly → na mapě je pravoúhlá sotva polovina budov. Orthogonalizace obrysu (dominantní osa + snap hran na násobky 90°, příp. min-area bounding rectangle u malých). **Nezávislé na L2 displacementu** (ten translatuje tuhý ring, tvar neřeší); logicky L1 (tvar) před/vedle L2 (poloha). Metoda k probrání s uživatelem (jeho noty). Větší vizuální návratnost než displacement.
- [ ] *(reframe Sez. 23)* Přejmenovat `generate()` → **`synthesize_pseudorealistic_map(n, e, w_km, h_km)`** (real-větev = prediktor mapy). Samostatný refaktor (reference v `batch.py`, `diagnose_displacement.py`, docs). Vize + dvoufázový model (projekce → AI predikce): IDEAS „synthesize_pseudorealistic_map".
