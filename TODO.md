...
- [~] **Kartografická generalizace — Úroveň 2: displacement** (`%THINK` Sez. 21 hotov, krok 0 změřen): odsazení budov od cest a mezi sebou na min. mezeru 0,4 mm (ISOM, ≈1,83 px). **Krok 0** (`diagnose_displacement.py`): ~28/99 budov v kolizi (Č. Švýcarsko), **dominuje budova↔cesta, shluky budov ~0** → zúženo na **greedy kolmé odsazení od pevné sítě** (cesty+voda = kotva), budova↔budova symetricky, malý strop, 1–2 iterace (ne NP-hard relaxace). **Zbývá:** (a) pipeline inverze kontroly u real vrstev (sběr geom → resolve → draw); (b) `resolve_displacement`; (c) GT konzistence (posun na geometrii → render+maska+OMAP). Detail: IDEAS.md „Úroveň 2".
