...
1. **K1 ÚHÚL věk porostu = ⟲ ARCHIVOVÁNO Sez. 82, kód SMAZÁN Sez. 102** (`connectors/forest.py` +
   `--forest-age`). Implementováno Sez. 62 jako hrubý PROXY (AOPK „Les_Mapy" vrstva 19 „Porostní skupiny",
   atribut `BARVA`=věk → ISOM zeleň 406/408/410, `gis.nature.cz`). **A1 measure-first ho vyřadil** (Sez. 82):
   pokrytí jen 33 % korpusu, IoU s kresbou kartografa 0,12, přestřel zelené 3,3×, pokrytí DĚRAVÉ 3/5 DEV
   (SV/HS 0). JEDINÝ zdroj predikční vegetace = **separace z reálné mapy** (`separate.py`); proxy = doložená
   slepá ulička (git/diář ji drží). Probe + KSLH PDF: `temp/uhul_probe/` (pokud zachováno).
