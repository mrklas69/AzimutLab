...
| `Stupeň__sráz` | linie | 104 Earth bank | ✓ | **Sez. 43** (`--linefeatures`, id 95 → 104 plná linie + jednostranné ticky; zemní sráz, NE skalní 201). **Σ981 = byla nejčastější netáhnutá vrstva** (od Sez. 43 táhnuta přes `--linefeatures` → 104). SV 71 / LS 393 / HS 377 |
| `Rokle__výmol` | linie | 107 Erosion gully | ✓ | **Sez. 58** (`--linefeatures`, id 94; polyline, jen `fid_zbg` → KISS 107 ne drobné 108; plná hnědá linie, mirror sráz 104 bez ticků). **Σ0 na 5 DEV** (erozní rýhy = měkké půdy, ne lesy Liberecka) → implementováno naslepo + ověřeno na výseku u Moravské Třebové (nejhustší shluk ČR, 32 roklí; OOM Test OK). V ČR celkem 1388 (řídká). Připraveno pro úplnost (mirror vodojem věžový Sez. 43) |
...
| `Významný_nebo_osamělý_strom__lesík` | bod | 417 Prominent large tree | ✓ | **Sez. 43** (`--landmarks`, `LANDMARK_POINT_LAYERS_417`; osamělý strom = OB objekt, zelený kroužek). Drift v katalogu (◐) opraven Sez. 55 — implementace existuje (`map_landmark_to_isom` → 417). **Sez. 136: zdroj ŘÍDKÝ (~3 % reálné hustoty 27/km²) → pseudo DOPLNĚN** na cíl (`_generate_pseudo_veg_points`, `pseudorealistic`). **419 Prom. veg. feature (zelený X) NEMÁ ZABAGED zdroj → čistě pseudo** (~18/km²); 418 Prom. bush = doložená KOMPAS díra (Příště). |
...
| *(plot)* | — | 516 Fence | **pseudo (Sez. 98)** | **ZABAGED plot nevede jako vrstvu** (doložený SKIP Sez. 57 — probe MapServeru vrací jen Zeď 39 / Hradba 38 / Zábrana 54). **Sez. 98: 516 kreslen jako PSEUDOREALISTICKÁ dekorace (fáze 2)** — ne z dat, ale po obvodu souvislých bloků zástavby (RÚIAN privát → olivová 520 dissolve), kde je oplocení věrohodné. Práh 0,5 ha (kalibr. proti reálným mapám, gen≈orig 24); ticky DOVNITŘ (ISOM spec „tags inside"). 517 Ruined / 518 Impassable fence stále SKIP (data nejsou). |
