...
- [!] **Doplnit CHYBĚJÍCÍ relevantní vrstvy ZABAGED** (Sez. 23, uživatel „stojíme o všechna data z geoportálu, ne jen vybraná"). **Katalog VŠECH 149 vrstev: `docs/kb/zabaged-isom-catalog.md`** — akční seznam kandidátů (◐). **HOTOVO: el. vedení 510 + sloupy (Sez. 24); koupaliště `Pozemní_nádrž`→301 (Sez. 27, Lesní koupaliště LS chybělo, je nádrž ne `Vodní_plocha`); řopíky `Bunkr` LO37 = asset (Sez. 27).** **PŘÍŠTĚ (fokus Sez. 28): železnice** (`Železnice`/`Tramvajová_dráha` → ISOM **509**, real-půlka, liniová — izomorfní s cestami). Pak `Most` (512, linie) + `Lávka` (512.2), `Osamělý_balvan`/`Skalní_útvary` (204/201), `Lesní_průsek` (508), bodové prvky (kříž 530/mohyla 526/věž 524), `Zeď` (513). Každý = vrstva + mapování + render + verify.
