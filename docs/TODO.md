...
- [ ] *(rychlá výhra, tvrdá data — Příště Sez. 99 bod 1)* **ZABAGED marsh varianty 310 indistinct / vodopád 313**
  na `--marsh` — measure-first geometrie v `resources/*.omap` (area vs point; Sez. 96 varoval „single dot → bod") → konektor, do stropu 54 %.
- [ ] *(linie, největší díra — Příště Sez. 99 bod 2)* **416 Distinct vegetation boundary** (orig 633 / gen 0, kompas)
  z hranic separovaných ploch 403/406/408, do statistické míry (reálné hranice vegetace většinou nejasné, IDEAS C Sez. 96).
- [ ] *(bug, nález Sez. 98)* **meta.json `real_sections: []` u predict cesty** (`measure_dod._gen_sep` / `generate_map`
  s `predict_areas_sjtsk`) — meta neeviduje predikční veg_area (objeveno při dotazu na původ 403 v `maps/Blatná`). Prozkoumat zápis meta.
