...
`generator()`, and **three live reconstructors**. Generator KPI is **68.7% canonical (HAL3000, session 176)**
after re-measurement (67.5% → 68.0%) plus a watercourse 304/305 classification fix (length per `idvt`
instead of named-flag; lines 66.8 → 68.7); **ntbhej 68.4%** (session 173) is a separate machine baseline,
stale w.r.t. the watercourse fix.
...
112 Pit (▽ filled) = brown terrain points**, joining 109 disk; real transfer **111 strong 0.71-0.89 (better than
109), 112 medium-good 0.53-0.77**; brown TRANSFERS like black/green despite competing with contour lines;
per-class peak_thr 0.60. **112 is reconstructor-only — generator does not draw it, kept out of capability/USED_CODES
so it does not create a false KPI hole**), and
...
model/                 # UC5 model code (sibling of connectors/generator, sys.path scripts; session 77). Four subdirs, three live reconstructors:
  ...
  png2point/           #   LIVE Png2Point reconstructor (sessions 105–106): map scan → point symbols, second of the 3 CV tasks
    inject.py          #     inject ISOM icons onto clean point_base render → GT for free + Gaussian heatmap splat (scope 204 Boulder / 210 Stony / 417 large tree / 419 veg. feature / 531 man-made X / 525 tower ⊤ / 527 fodder Λ / 109 brown knoll disk / 111 brown depression ∪ / 112 brown pit ▽; halo flag: green halo yes, black/brown no)
